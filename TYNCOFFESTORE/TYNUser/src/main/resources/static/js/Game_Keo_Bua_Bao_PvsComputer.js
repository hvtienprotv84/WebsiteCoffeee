...
function random() {
  let randomEl =
    "hinhanh_game_keo_bua_bao/" + imgArray[Math.floor(Math.random() * imgArray.length)];
  if (computerChoice.getAttribute("src") !== randomEl) {
    return randomEl;
  } else {
    return "hinhanh_game_keo_bua_bao/" + imgArray[Math.floor(Math.random() * imgArray.length)];
  }
}
